// ============================================================
// ESL 296x128 BWR — Dooray 4 Rooms (개선 통합 버전 v4.1)
// - 매 정시/30분 강제 업데이트 (시계 항상 갱신)
// - Dooray 실패해도 화면(시계/상태) 재그림
// - 캔버스 1회 생성 후 재사용(힙 파편화 방지)
// - ISO8601 타임존 보정(Z, ±hh:mm) 처리
// - JSON 여유 증가, Wi-Fi deinit 안정화, 기타 마이크로 최적화
// ============================================================
...
// Dooray 개인 API 토큰 (필수, 임시 값)
static const char* DOORAY_TOKEN = "YOUR_DOORAY_TOKEN_HERE";
...
static GFXcanvas1* canvasBW = nullptr;   // 상주
static GFXcanvas1* canvasRED = nullptr;  // 상주
...
static time_t parseISO(const char* iso, uint8_t& h, uint8_t& m){
  ...
  const char* tzp = iso + 19; // 'Z' or +hh:mm/-hh:mm 위치

  if (*tzp == 'Z') { sign = 0; /* UTC */ }
  else if (*tzp == '+' || *tzp == '-') {
    sign = (*tzp == '+') ? +1 : -1;
    if (sscanf(tzp+1, "%2d:%2d", &offH, &offM) != 2) { offH = 0; offM = 0; sign = 0; }
  }
  ...
  // 입력이 UTC('Z') 또는 오프셋 포함이면 KST(+9) 기준으로 보정
  if (*tzp == 'Z' || sign != 0) {
    // KST(+9) - (입력 오프셋)
...
static void drawScreen(const Tag& tag, const RoomData& rd){
  ...
  canvasBW->fillScreen(1);  // 흰색
  canvasRED->fillScreen(1); // 흰색
  ...
  if(rd.count == 0){
    ...
    for(int i=0; i<shown; i++){
      ...
      // 이름 처리
      char who[36];
      strlcpy(who, rd.list[i].who, sizeof(who));
      if(strlen(who) > 24) {
        who[22] = '.'; who[23] = '.'; who[24] = '\0';
      }

      // 제목 처리
      char subj[80];
      strlcpy(subj, rd.list[i].subj, sizeof(subj));
      if(strlen(subj) > 42) {
        subj[40] = '.'; subj[41] = '.'; subj[42] = '\0';
      }
...
static void wifiOff(){
  ...
  delay(300);
  esp_wifi_stop();               // 이미 stop돼 있어도 OK
  delay(200);
  esp_err_t e = esp_wifi_deinit();
  (void)e; // 에러여도 무시 (상태 레이스 방지)
  delay(200);
  WiFi.mode(WIFI_OFF);
  delay(500);
}
...
static void processUpdates(bool forceUpdate = false) {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    // 그래도 시계 갱신을 위해 아래로 진행할 수 있게 하려면
    // Dooray fetch는 스킵되지만 화면은 다시 그림(아래에서 처리)
  } else {
    ensureTime();
  }
  ...
  for(int i = 0; i < NUM_TAGS; i++) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      // 네트워크 없음: 빈 리스트 + ok=false 유지
      g_room[i].count = 0;
      g_room[i].ok = false;
      g_room[i].dataHash[0] = '\0';
    }

    if (!fetchOk) {
      Serial.println("FAIL (will still refresh screen)");
    } else {
      Serial.println("OK");
    }

    // 현재 회의 상태/변경 판단 (fetch 실패시 count=0 → busy=false)
  ...
  if (updateCount > 0) {
    ...
    for(int i = 0; i < NUM_TAGS; i++) {
      ...
      for(int attempt = 0; attempt < 3 && !uploadOk; ++attempt) {
        ...
        if (!uploadOk && attempt < 2) {
          delay(1800); // 소백오프
        }
      }

      if (uploadOk) {
        // fetch 성공시에만 lastHash 갱신 (실패면 hash 유지 → 다음에 다시 시도)
        if (g_room[i].ok) {
...
void setup(){
  ...
  Serial.println("\n╔════════════════════════════════════╗");
  Serial.println("║   ESL Dooray Final v4.1 (Improved) ║");
  Serial.println("║   정시/30분 강제 스마트 업데이트   ║");
  Serial.println("╚════════════════════════════════════╝");

  // 캔버스는 1회 생성 후 계속 재사용 (힙 파편화 방지)
...
void loop() {
  delay(30000);  // 30초 폴링(정시/30분 트리거 놓치지 않음)
  ...
  if (timeReady()) {
    ...
    if (lt.tm_min == 0 || lt.tm_min == 30) {
      ...
      if (lt.tm_hour != lastHour || lt.tm_min != lastMin) {
        lastHour = lt.tm_hour;
        lastMin  = lt.tm_min;
        Serial.printf("\n[TIME] %02d:%02d - Starting update (force)\n", lt.tm_hour, lt.tm_min);
        processUpdates(true);   // 무조건 업데이트 → 시계/상태 항상 갱신
      }
    }
  }
